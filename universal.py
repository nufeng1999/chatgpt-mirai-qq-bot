...
_fkflag:int=0
_fktime=time.time()
def set_fkflag(flag:int):
    global _fkflag
    _fkflag=flag

def chkfk(session_id:str):
    global _fkflag
    global _fktime
    if not session_id.startswith("group"):
        return 0
    timestamp1 = time.time()
    if _fktime < timestamp1 - 1 * 60 * 60:
        _fkflag=0
    return _fkflag
#
...
async def handle_message(
    _respond: Callable,
    _rpcrespond: Callable,
    session_id: str,
    message: str,
    chain: MessageChain = MessageChain("Unsupported"),
    is_manager: bool = False,
    nickname: str = '某人',
    request_from=BotPlatform.AriadneBot):
    ...
    def wrap_request(n, m):
        ...
        async def call(session_id, message, conversation_context, respond):
            await m.handle_request(session_id, 
                                   message, 
                                   respond, 
    ...
    async def respond(msg: str):
        ...
        if not msg:
            return
        logger.debug(f"respond msg:{session_id},{msg}")
        if chkfk(session_id)==0:
            oldmsg=msg
            ret = await _respond(msg)
        else:
            await _rpcrespond(msg)
            return
    ...
    try:
        ...
        # else:
            # await _rpcrespond(config.response.rpcforward_fkinfo)
        ...
        #logger.exception(e)
        set_fkflag(1)
        await _rpcrespond(oldmsg)
        #await _rpcrespond(config.response.rpcforward_fkinfo)
        # await _respond("哎！又被风控中，暂时无法回复你的内容了。")
