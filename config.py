...
class TongyiCookiePath(BaseModel):
    TOKEN: str
    ssoSessionId: str
    """通义 Cookie 中的 ssoSessionId 字段"""
    fd: Optional[str] = ""
    """通义请求中的 fd 字段"""
    GtToken: Optional[
...
class XinghuoCookiePath(BaseModel):
    ...
    auto_remove_old_conversations: bool = False
    """自动删除旧的对话"""
class Xinghuo2_0_CookiePath(BaseModel):
    APPID: str
    APISecret: str
    APIKey: str
    proxy: Optional[str] = None
    """可选的代理地址，留空则检测系统代理"""
class Xinghuo1_5_CookiePath(BaseModel):
    APPID: str
    APISecret: str
    APIKey: str
    proxy: Optional[str] = None
...
class TongyiAuths(BaseModel):
    accounts: List[TongyiCookiePath] = []
    """通义千问大模型的账号列表"""
class XinghuoAuths(BaseModel):
    accounts: List[XinghuoCookiePath] = []
    """讯飞星火大模型的账号列表"""
class Xinghuo2_0_Auths(BaseModel):
    accounts: List[Xinghuo2_0_CookiePath] = []
    """讯飞星火大模型的账号列表"""
...
class Response(BaseModel):
    ...
    rpcforward_fkinfo: str = "哎！它又被风控中，暂时无法回复你的内容了。可以和它私聊，这样它可以回复你的内容或问我。"
...
class Config(BaseModel):
    ...
    tongyi: TongyiAuths = TongyiAuths()
